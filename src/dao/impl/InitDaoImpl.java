        ...
        for(int i = 0;i < space;i++) {
            switch (op[i].type) {
                case "blank"    :
                    ...
                    expression.append(op[i].pocket);
                    break;
                case "bracket" :
                    System.out.print(op[i].pocket);
                    expression.append(" ").append(op[i].pocket).append(" ");
                    break;
                case "oper"    :
                    System.out.print(op[i].operator);
                    expression.append(op[i].operator);
                    break;
                case "num"     :
                    op[i].print();
                    if(op[i].integer > 0) {
                        expression.append(op[i].integer).append("'").append(op[i].numerator).append("/").append(op[i].denominator);
                    } else if(op[i].denominator==1) {
                        expression.append(op[i].numerator);
                    } else {
                        expression.append(op[i].numerator).append("/").append(op[i].denominator);
                    }
                    break;
                default:break;
            }
